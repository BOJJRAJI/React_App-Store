...
class AppStore extends Component {
  state = {tabId: tabsList[0].tabId, inputValue: ''}
  ...
  render() {
    const {inputValue, tabId} = this.state
    const filteredAppsList = this.filterApps()
    const filterBasedOnInputValue = filteredAppsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(inputValue.toLowerCase()),
    )
    console.log(filterBasedOnInputValue)

    return (
      <div className="bg-container">
        <div className="card">
          <h1 className="heading">App Store</h1>
          <button className="input-container" type="button">
            <input
          ...
          <ul className="tabs-container">
            {tabsList.map(eachTab => (
              <TabItem
                tabDetails={eachTab}
                key={eachTab.tabId}
                onChangeTab={this.onChangingTab}
                isActive={tabId === eachTab.tabId}
              />
            ))}
          </ul>
          <ul className="apps-container">
            {filterBasedOnInputValue.map(eachApp => (
